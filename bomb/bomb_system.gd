...
# Timestamp of the last real ScreenTouch press, used to suppress synthetic mouse events.
...
func _input(event: InputEvent) -> void:
	...
	# Ignore synthetic MouseButton events generated by the browser after a touch tap.
	if is_mouse and now - _last_touch_event_time < TOUCH_TO_MOUSE_SUPPRESS_SECS:
		return
	...
	# Use event.position for touch (accurate); fall back to mouse position for mouse.
	var pos: Vector2 = (event as InputEventScreenTouch).position if is_touch else get_viewport().get_mouse_position()
	var time_delta := now - _last_tap_time

	if _last_tap_time >= 0.0 and time_delta <= DOUBLE_TAP_TIME:
		var dist := pos.distance_to(_last_tap_position)
