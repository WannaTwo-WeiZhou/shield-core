...
- 能力定义中已**移除标签（tags）系统**；联动条件统一使用 `required_abilities` 精确匹配能力 ID。
...
> 说明：能力定义不再包含 `tags / affects_tags / responds_to_tags` 字段。联动条件请通过 `required_abilities` 精确指定能力 ID。
...
- 条件字段：
  - `required_abilities`：按能力 ID 精确匹配（**唯一触发方式**，至少包含 1 个 ID，组合联动一般填 2 个或更多）。
- 效果字段：
  - `effect`：单效果（兼容写法）。
  - `effects`：多效果数组（推荐写法）。
...
1. 在 `ability/abilities_config.json` 新增能力条目，补齐 `id`、`per_level` 等字段。
2. 确认能力类型：
   - 纯属性型：复用已有属性键时无需额外代码。
...
4. 如需做“可重复能力”，设置 `repeatable: true`，并谨慎评估是否将其加入任何联动的 `required_abilities`，避免预期外的反复激活。

### B. 新增能力融合（2 能力组合）

1. 在 `ability/synergies_config.json` 新增联动条目。
2. 使用 `required_abilities` 指定两个能力 ID（联动的唯一触发方式）。
